...
from diffusers import FluxFillPipeline, FluxTransformer2DModel, FluxPriorReduxPipeline
...
REDUX_MODEL_ID   = "black-forest-labs/FLUX.1-Redux-dev"
...
def build_pipeline():
    print("Initializing FLUX.1 Fill  |  NF4 4-bit  |  TeaCache...")
    ...
    pipe.vae             = pipe.vae.to(GPU_MAIN, dtype=DTYPE)
    pipe.vae.use_tiling  = False
    pipe.vae.use_slicing = False
    pipe.set_progress_bar_config(disable=True)

    print(f"  Loading FLUX.1 Redux prior pipeline → {GPU_MAIN}...")
    ...
    def _timed_vae_decode(*args, **kwargs):
        torch.cuda.synchronize()
        t0 = time.perf_counter()
        out = _orig_vae_decode(*args, **kwargs)
        torch.cuda.synchronize()
        pipe._last_vae_decode_time = time.perf_counter() - t0
        return out
    pipe.vae.decode = _timed_vae_decode
    pipe._last_vae_decode_time = 0.0

    print(f"  Compiling transformer (mode={COMPILE_MODE or 'default'})...")
...
def run_warmup(pipe):
    dummy_img  = Image.new("RGB", (DEFAULT_WIDTH, DEFAULT_HEIGHT), color=(128, 128, 128))
    dummy_mask = Image.new("L", (DEFAULT_WIDTH, DEFAULT_HEIGHT), color=0)
    box = np.array(dummy_mask)
    h, w = box.shape
    box[h // 4: 3 * h // 4, w // 4: 3 * w // 4] = 255
    dummy_mask = Image.fromarray(box)

    pe, ppe = encode_prompt(pipe, "warmup prompt", dummy_img)

    orig = apply_teacache(pipe, DEFAULT_STEPS, REL_L1_THRESH)

    print("  Warming up (this compiles the graph, will be slow once)...")
    t0 = time.perf_counter()
    ...
    with torch.inference_mode():
        result = pipe(
            ...
            generator=torch.Generator(device=GPU_MAIN).manual_seed(SEED),
        )
...
def encode_prompt(pipe, prompt: str, reference_image: Image.Image):
    t0 = time.perf_counter()
    with torch.inference_mode():
        redux_out = pipe.prior_redux(image=reference_image, prompt=prompt)
    pe  = redux_out.prompt_embeds.to(GPU_MAIN, dtype=DTYPE)
    ppe = redux_out.pooled_prompt_embeds.to(GPU_MAIN, dtype=DTYPE)
    print(f"  [redux] encode time: {time.perf_counter()-t0:.2f}s")

    print(f"  [shape check] prompt_embeds:        {tuple(pe.shape)}  dtype={pe.dtype}")
    print(f"  [shape check] pooled_prompt_embeds:  {tuple(ppe.shape)}  dtype={ppe.dtype}")
    return pe, ppe
...
def load_input_mask(source: str) -> Image.Image:
    if source.startswith("http"):
        mask = load_image(source).convert("L")
    else:
        mask = Image.open(source).convert("L")
    return mask.resize((DEFAULT_WIDTH, DEFAULT_HEIGHT), Image.NEAREST)
...
def generate(
    ...
    """
    reference_image_source : the "prompt image" 
    image_source            : the room photo actually being inpainted 
    mask_source              : white = region to fill, black = keep as-is.
    """
    t_load = time.perf_counter()
    ref_img = load_input_image(reference_image_source)
    img     = load_input_image(image_source)
    mask    = load_input_mask(mask_source)
    print(f"  ✓ Reference + image + mask loaded ({time.perf_counter()-t_load:.1f}s)")

    t_txt   = time.perf_counter()
    pe, ppe = encode_prompt(pipe, prompt, ref_img)
    ...
    with torch.inference_mode():
        ...
        result = pipe(
            ...
            width=DEFAULT_WIDTH,
            max_sequence_length=MAX_SEQ,
            generator=torch.Generator(device=GPU_MAIN).manual_seed(SEED + job_index),
        )
        torch.cuda.synchronize()
        t_diffusion = time.perf_counter() - t_diff0
    ...
    t_enhance   = time.perf_counter() - t_enh
    ...
    print(f"  Skipped     : {skipped}/{DEFAULT_STEPS} steps ({100*skipped//DEFAULT_STEPS}%)  "
          f"(max_consecutive={MAX_CONSECUTIVE_SKIPS})")
    vae_t = getattr(pipe, "_last_vae_decode_time", 0.0)
    print(f"  Diffusion   : {t_diffusion:.2f}s  "
          f"(transformer loop ~{t_diffusion - vae_t:.2f}s + VAE decode {vae_t:.2f}s)")
    print(f"  Enhancement : {t_enhance:.3f}s")
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 60)
    print("  FLUX.1 Fill  |  L4  |  NF4  |  TeaCache")
    print(f"  Steps={DEFAULT_STEPS}  Guidance={DEFAULT_GUIDANCE}  "
          f"Res={DEFAULT_WIDTH}×{DEFAULT_HEIGHT}  thresh={REL_L1_THRESH}  "
          f"MaxSeq={MAX_SEQ}  compile={COMPILE_MODE or 'default'}")
    print("  Target: <15s latency @ 1024x1024")
    ...
    while True:
        ...
        reference_image_source = input(
            "  Prompt image (reference style, URL or local path): "
        ).strip()
        if reference_image_source.lower() in ("quit", "exit"):
            break

        image_source = input("  Image to replace in (URL or local path): ").strip()
        if image_source.lower() in ("quit", "exit"):
            break

        mask_source = input("  Mask (URL or local path, white=fill area): ").strip()
        if mask_source.lower() in ("quit", "exit"):
            break

        prompt = input("  Prompt (optional, can be empty): ").strip()
        if prompt.lower() in ("quit", "exit"):
            break
        prompt = prompt or None

        generate(pipe, prompt, reference_image_source, image_source, mask_source, job_index)
        job_index += 1
